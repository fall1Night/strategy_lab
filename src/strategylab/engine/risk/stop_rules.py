# -*- coding: utf-8 -*-
"""止损/止盈规则引擎（移植自 deepseek-harness-quant risk/stop_rules.py，MIT）。

RiskAgent 管"能不能买"（一票否决），本模块管"什么时候卖"（铁律执行）。

纪律（不可协商）：任何卖出规则触发 → 次日开盘执行（T+1，防 Look-Ahead）。

规则清单：
1. 硬止损：亏损 ≥ 7% 无条件卖出（欧奈尔铁律）
2. 时间止损：买入 3 周内涨幅 < 3% 卖出（判断错误）
3. 支撑止损：收盘跌破突破点下方 8%
4. ATR 止损：止损价 = 入场价 - 2×ATR（软止损取更严者）
...
class StopRules:
    ...
    def __init__(self, config: dict | None = None):
        cfg = config or {}
        self.stop_loss_pct = cfg.get("stop_loss_pct", 0.07)          # 硬止损 7%
        self.stop_loss_below_pivot = cfg.get("stop_loss_below_pivot", 0.08)  # 突破点下方 8%
        self.time_stop_weeks = cfg.get("time_stop_weeks", 3)          # 时间止损 3 周
        self.time_stop_min_gain = cfg.get("time_stop_min_gain", 0.03)  # 3 周涨幅 < 3%
        self.profit_take_low = cfg.get("profit_take_low", 0.20)       # 止盈下沿 20%
        self.profit_take_high = cfg.get("profit_take_high", 0.25)     # 止盈上沿 25%
        self.trailing_stop_ma = cfg.get("trailing_stop_ma", 50)       # 移动止损均线
        self.high_drawdown_exit = cfg.get("high_drawdown_exit", 0.08)  # 高点回撤 8%
        self.atr_stop_mult = cfg.get("atr_stop_mult", 2.0)            # ATR 倍数

    def check(self, *, entry_price: float, current_price: float,
              pivot_price: float | None = None, atr: float | None = None,
              ma_50: float | None = None, peak_price: float | None = None,
              hold_weeks: float = 0.0, take_profit_ratio: float | None = None
              ) -> StopDecision:
        """检查单个持仓的所有卖出规则。
...
if __name__ == "__main__":
    ...
    scenarios = [
        ...
        ("硬止损触发", dict(entry_price=10, current_price=9.2, hold_weeks=2)),
        ("时间止损", dict(entry_price=10, current_price=10.2, hold_weeks=4)),
        ("ATR止损", dict(entry_price=10, current_price=9.2, atr=0.35, hold_weeks=2)),
        ("跌破50日线", dict(entry_price=10, current_price=9.8, ma_50=9.9, hold_weeks=2)),
        ("高点回撤", dict(entry_price=10, current_price=10.2, peak_price=11.2, hold_weeks=2)),
        ("止盈20%分批", dict(entry_price=10, current_price=12.2, hold_weeks=2)),
        ("止盈25%全出", dict(entry_price=10, current_price=12.8, hold_weeks=2)),
    ]
    print("=== 止损/止盈规则引擎测试 ===")

# -*- coding: utf-8 -*-
"""假突破鉴别器（移植自 deepseek-harness-quant risk/fake_signal_detector.py，MIT）。

★背景：A 股突破胜率仅 45.5%（17 年实证），近一半突破是假的（诱多）。
★用途：突破机会入池/持仓时鉴别真突破 vs 假突破 → 降级或触发止损。
...
class FakeSignalDetector:
    ...
    def assess(self, *, break_pct: float, vol_ratio: float,
               vol_next_ratio: float | None = None,      # T+2 量 / 突破日量
               upper_shadow_ratio: float | None = None,  # 上影 / 实体
               sector_sync: int | None = None,           # 同行业同时突破数
               close_vs_pivot: float | None = None,      # 当前价 / 突破点 - 1

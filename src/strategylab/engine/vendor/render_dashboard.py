...
def build_dashboard_data(
    ...
    if extra_modules:
        ...
        # 若提供了合并持仓表(position_table), 默认平铺 trades_table 冗余, 移除之
        if any(m.get("type") == "position_table" for m in extra_modules):
            report_data["modules"] = [
                m for m in report_data["modules"] if m.get("type") != "trades_table"
            ]
    return report_data
...
def render_dashboard(
    ...
    if template_path:
        ...
        template_resource = resources.files("strategylab.engine.vendor").joinpath(
            "dashboard_template.html"
        )
        template = template_resource.read_text(encoding="utf-8")
        template_file = template_resource
...
def _build_default_modules(
    ...
    # 条件列: 交易带 role 字段(底仓/做T)时显示「类型」列, 向后兼容其它回测
    if any(_first_non_empty(trade, "role") for trade in trade_history):
        _role_label = "类型" if language == "zh" else "Role"
        trade_columns.append({"key": "role", "label": _role_label, "format": "pill"})
    trade_columns.extend([

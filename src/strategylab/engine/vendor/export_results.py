...
def export_results(
    ...
    """Compute backtest results in-memory, DB-first by default.

    Default ``write_files=False``: no file is written; the defensive checks
    still run and the summary is computed, then returned in-memory for the DB
    layer (``storage.repository.save_run``) to persist. Pass ``write_files=True``
    only for an explicit local-file export, in which case the three standard
    files (``<prefix>_equity.csv`` / ``<prefix>_trades.csv`` /
    ``<prefix>_summary.json``) are written to disk as before.

    Returns:
        A dict. Always contains ``summary_data`` (the computed summary dict) and
        ``meta``. When files are written, also contains ``equity`` / ``trades`` /
        ``summary`` paths; otherwise those keys are ``None``.
    """
    ...
    # ---- Optionally write the 3 output files (DB-first mode skips this) ----
    result: dict[str, Any] = {"summary_data": summary, "meta": meta}
    if write_files:

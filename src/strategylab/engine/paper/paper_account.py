# -*- coding: utf-8 -*-
"""模拟盘模块（移植自 deepseek-harness-quant strategy/paper_account.py，MIT）。

虚拟账户：模拟成交/持仓/净值，SQLite 持久化。
回测与未来实盘共用同一套下单/持仓接口（单一路径，防"模拟一套实盘一套"）。

★适配 strategy_lab：db_path 必填（不再写死默认路径），其余接口原样保留。

设计要点：
- 撮合规则（A股近似）：T+1 卖出限制、涨跌停无法成交（±10%/ST±5% 简化用 10%）、
  成交价=当日收盘价（低频调仓下收盘撮合足够，滑点由成本模型覆盖）
- 数据持久化：tables = accounts / positions / orders / equity_curve
- 幂等：同一 (account, date, code, action) 不重复记录

用法：
  acc = PaperAccount("demo", cash=1_000_000, db_path="data/paper.db")
...
class PaperAccount:
    ...
    def __init__(self, name: str = "default", cash: float = 1_000_000,
                 db_path: str | Path | None = None, start_date: str | None = None):
        if db_path is None:
            raise ValueError("PaperAccount 需要显式 db_path（持久化位置）")
        self.name = name
        self.cash = cash

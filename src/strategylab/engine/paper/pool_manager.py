# -*- coding: utf-8 -*-
"""三池管理（移植自 deepseek-harness-quant strategy/pool_manager.py，MIT）。
...
class PoolManager:
    ...
    def __init__(self, db_path: str | Path | None = None):
        if db_path is None:
            raise ValueError("PoolManager 需要显式 db_path（持久化位置）")
        self.db = str(db_path)
        self._init()
    ...
    def update_pools(self, rank_df, buy_signals=None, holdings_out=None, date=None):
        """每日/每周更新三池流转。
        rank_df: 排名引擎输出（候选来源，需含 code/综合分 列）
...
if __name__ == "__main__":
    ...
    import os
    print("=== 三池管理自测 ===")
    tmp = os.path.join(tempfile.gettempdir(), "pools_test.db")
    if os.path.exists(tmp):
        os.remove(tmp)
    pm = PoolManager(db_path=tmp)
    import pandas as pd

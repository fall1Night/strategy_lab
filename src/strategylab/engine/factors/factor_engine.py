# -*- coding: utf-8 -*-
"""因子计算引擎（移植自 deepseek-harness-quant factors/factor_engine.py，MIT）。

依据：M3 因子验证 + 市场简单回测实证 + 中证指数官方研究（CS-01/02/03）。
...
| mom_20 | 20日涨幅 | -1（短期反转） | A股短期反转显著 |
| mom_120 | 120日涨幅 | -1（小盘池反转） | CS-03 分池 |
| rps_120 | 相对强度 | -1（降权反转） | CS-01/03 |
| new_high_250 | 52周新高 | 0（剔除） | 各口径均为负 |
"""
...
def compute_factor_panel(closes: pd.DataFrame, direction: dict | None = None,
                         factors: list | None = None) -> pd.DataFrame:
    """计算全市场因子面板（方向化后，越大越好）。
    ...
    for name in factors:
        ...
        if sign == 0:
            continue  # 已剔除因子
        # 向量化：整 DataFrame 一次算（shift/rolling/pct_change 天然支持矩阵）
...
def direction_summary(direction: dict | None = None) -> pd.DataFrame:
    """打印当前因子方向表（决策依据）"""
    direction = direction or DEFAULT_DIRECTION
    rows = []

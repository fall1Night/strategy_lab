...
- **回测"已更新数据源却仍联网"根因（设计行为非 bug）**：`backtest.run_symbol` 的 `fetch_end=max(end_ts, today)`，且 verify 判定靠 `cache._covers(meta,beg,end)` 要求 **请求区间 ⊆ 缓存区间**（即 `mb<=beg and me>=end`）。回测 `required_end` 被强制拉到**今天**，而"更新数据源"写到的 akshare 缓存末日常见滞后（实测 2026-07-28 当天：日线 meta 多为 0719/0720/0721，周线多为 0724/0719）。**缓存末日 < 今天 → `_covers` 返 False → 抛 DataMissingError → 降级 `mode="update"` 联网补最近几天（走 akshare=新浪）**。故"明明更新过还联网"是 verify 必须覆盖到今天的设计所致；不是前缀错配（disk 绝大多数是 akshare，与 `.env` 默认 akshare 一致）。优化方向：用 `--end $(date +%Y%m%d)` 显式拉到今天 / 更新源确认拉到今天 / verify 容忍末日容差（如允许末日 ≥ 今天-7）。
